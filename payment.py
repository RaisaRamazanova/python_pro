...
async def buy(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # )
    currency = "RUB"
    prices = [LabeledPrice(label='Покупка {level} тарифа'.format(level="Test"), amount=1000).to_dict()]
...
async def handle_pre_checkout(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.invoice_payload != '{section}-{level}-payload'.format(section="Test", level="Test"):
